...
class AssertionHelper {
    ...
    _prettyJson(json, indentation, color) {
        ...
        if (indentation && (me._isJSON(json) || (_.isArray(json) && json.length > 0))) {
            let jsonStrArr = jsonStr.divideit("\n");
                ...
                if (me._isJSON(aVal)) {
                    ...
                    if (aVal.length == 0 && eVal.length == 0) {
                        ...
                        if (me._isJSON(aVal[0])) {
                            ...
                            while(i<aVal.length) {
                                let diff = me._prettyJson(aVal[i], indentation + 4, acolor);
                                if(i==aVal.length-1) {
                                    acomma = "";
                                }
                                acutalJsonString += `<code style="background-color: ${resetColor}" >${space.repeat(indentation+4)}${diff[0]}${acomma}</code><br/>`;
                            ...
                            while(j < eVal.length) {
                                ...
                                let diff = me._prettyJson(eVal[j], indentation + 4, acolor);
                                expectedJsonString += `<code style="background-color: ${resetColor}" >${space.repeat(indentation+4)}${diff[0]}${acomma}</code><br/>`;
