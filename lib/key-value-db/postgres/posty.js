...
class PGModel {
  constructor (dependencies, tableName) {
  ...
  _constructFilterQuery(options, tableName) {
    let me = this;
    let filterQuery = 'SELECT {0}, created_date, modified_date FROM {1} WHERE ';
  ...
  filterTable(options) {
    const me = this;
    return me.tableFilter(this.pgp, options);
  }

  tableFilter (db, options) {
    const me = this;
    var startTime = (new Date()).getTime();
    return co(function * () {
      let command = me._constructFilterQuery(options, me.tableName);
      console.log(JSON.stringify(command, null, 10));
      let entities = yield db.any(command.text, command.values);
  ...
  tableInsert(email, value) {
    const me = this;
    return me._tableInsert(this.pgp, email, value);
  }

  _tableInsert (db, email, value) {
    const me = this;
      return co(function * () {
        let currentTime = new Date();
        let columnNames = _.values(Constants[me.tableName]);
        let command = `INSERT INTO ${me.tableName} (${columnNames.join(',')}) VALUES ($${1},$${2},$${3},$${4})`;
        let obj = me._serializeValue(value)
        let date = currentTime.toISOString();
        let input  = [
          email,
